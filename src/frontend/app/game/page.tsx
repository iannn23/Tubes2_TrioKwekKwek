...
import elementsData from '../../../backend/Scraper/elements.json';
...
export default function GamePage() {
  ...
  const [edgesDiscovered, setEdgesDiscovered] = useState<{id:string, source:string, target:string}[]>([]);
  ...
  useEffect(() => {
    ...
    if (!exactMatch && targetElement.length > 1) {
      const matches = validElements.filter(elem => 
  ...
  useEffect(() => {
    if (currentVisitIndex >= 0 && currentVisitIndex < nodeVisitSequence.length) {
      const timer = setTimeout(() => {
        ...
        if (searchResult && searchResult.treeStructure) {
          const newEdges = searchResult.treeStructure.edges
            .filter((edge: { target: string; source: string; id: string }) => 
              (edge.target === node && nodesDiscovered.includes(edge.source)) ||
    ...
    } else if (currentVisitIndex >= nodeVisitSequence.length && nodeVisitSequence.length > 0) {
      // Animation complete
      console.log("Tree visualization complete");
    }
  }, [currentVisitIndex, nodeVisitSequence, nodesDiscovered, searchResult, edgesDiscovered]);
  ...
  const handleSearch = async () => {
    ...
    if (!isValidElement) {
      alert("Elemen tidak valid! Gunakan elemen yang ada dalam database Little Alchemy 2.");
      return;
    }
    ...
    try {
      ...
      if (data.treeStructure && data.treeStructure.nodes) {
        const targetNode = data.treeStructure.nodes.find((n) => n.type === "target");
  ...
  const generateRecipePath = (treeStructure) => {
    ...
    // Find target node
    const targetNode = treeStructure.nodes.find(node => node.type === "target");
    if (!targetNode) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-950 via-green-900 to-green-800 text-white p-6">
      <h1 className="text-4xl font-bold mb-6 text-green-400">Pencarian Recipe Little Alchemy 2</h1>
        ...
        <div className="flex flex-col gap-2 relative">
          <input
            ...
            className={`bg-gray-900 border ${
              targetElement && !isValidElement ? 'border-red-500' : 'border-green-500'
          ...
          {targetElement && !isValidElement && (
            <p className="text-red-400 text-sm">
              Elemen tidak ditemukan. Pilih elemen yang valid dari Little Alchemy 2.
            </p>
          )}
        </div>
      </div>
        ...
        <p className="text-lg mb-4">
          ...
          {targetElement && (
            <>
              {" "}
              | Target:{" "}
              <span className={`font-semibold ${isValidElement ? 'text-green-400' : 'text-red-400'}`}>
                {targetElement}
              </span>
            </>
          )}
      ...
      {searchResult && (
        <div className="bg-black/50 rounded-lg p-4 border border-green-500 mb-6">
          <h3 className="text-xl font-semibold text-green-300 mb-2">Hasil Pencarian</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-gray-900/80 p-3 rounded-lg">
              <p className="text-gray-300">Jumlah Node yang Dikunjungi: 
                <span className="text-white font-semibold ml-2">{searchStats.totalNodes}</span>
              </p>
            </div>
            <div className="bg-gray-900/80 p-3 rounded-lg">
              <p className="text-gray-300">Waktu Eksekusi: 
                <span className="text-white font-semibold ml-2">{searchStats.executionTime} ms</span>
              </p>
            </div>
          </div>
            ...
            {searchResult && (
              <div className="overflow-auto max-h-96 text-sm">
                {searchResult.treeStructure ? (
                  <>
                    {(() => {
                      const generatedPath = generateRecipePath(searchResult.treeStructure);
                        ...
                        return (
                          <div className="text-yellow-300 mb-2">
                            ...
                            <div className="text-xs mt-1">
                              Nodes: {searchResult.treeStructure.nodes?.length || 0}, 
                              Edges: {searchResult.treeStructure.edges?.length || 0}
                            </div>
                          </div>
                        );
                      }
                      ...
                      return (
                        <div className="space-y-2">
                          {generatedPath.map((step, index) => (
                            <div key={index} className="p-2 border border-gray-800 rounded bg-gray-900/50">
                              <div className="font-mono">
                                <span className="text-green-400 font-semibold">Step {index + 1}: </span>
                                {step.result} (T{step.resultTier}) → {step.ingredients.map(ing => 
                                  `${ing.name} (T${ing.tier})`).join(' + ')}
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="mt-4 border-t border-gray-700 pt-3">
                      <div className="text-yellow-400 font-semibold mb-2">Using raw path data:</div>
                      <div className="space-y-1">
                        {searchResult.path && searchResult.path.map((step, index) => (
                          <div key={index} className="p-1 border-b border-gray-800">
                            <span className="text-yellow-400 font-semibold">Raw Step {index + 1}: </span>
      ...
      <div className="bg-gray-900 border border-green-500 p-6 rounded-lg shadow-lg">
        ...
        <RecipeTree 
          nodes={searchResult ? searchResult.treeStructure?.nodes.filter(
            node => nodesDiscovered.includes(node.id)
          ) : []} 
          edges={edgesDiscovered} 
          animationInProgress={currentVisitIndex < nodeVisitSequence.length && currentVisitIndex >= 0}
        />
      </div>
    </div>
  );
